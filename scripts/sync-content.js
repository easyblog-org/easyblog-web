...
function run(cmd) {
  try {
    ...
    // 输出 Git 的真实 stderr，方便查看错误原因（如分支不存在、空仓库等）
...
function main() {
  ...
  try {
    ...
    // 去掉 2>&1，让 Node 正常捕获 stdout/stderr
    run(`git clone --depth 1 --branch ${CONTENT_BRANCH} "${CONTENT_REPO}" "${TEMP_DIR}"`);
  } catch (e) {
    log('⚠️ Clone failed, see stderr above');
    if (fs.existsSync(TEMP_DIR)) run(`rm -rf "${TEMP_DIR}"`);
    if (currentHash !== 'empty') {
      log('Skipping sync - keeping current articles');
    } else {
      log('No local articles either, but continuing build (may be empty site)');
    }
    return;
  }
  ...
  function countArticles(dir) {
    ...
    fs.readdirSync(dir).forEach(f => {
      ...
      if (fs.statSync(fp).isDirectory()) n += countArticles(fp);
      else if (f.endsWith('.md')) n++;
    });
    return n;
  }
  const count = countArticles(ARTICLES_DIR);
